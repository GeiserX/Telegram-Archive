...
- **`DOWNLOAD_CHAT_DESCRIPTION` fetches each chat's description** (a group or channel's about text, a user's bio) and, for channels and supergroups, the member count the dialog list never carries, so the info panel can show them. Off by default: it is one extra request per chat per run.
