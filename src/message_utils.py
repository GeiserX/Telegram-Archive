...
def message_plain_text(message: object) -> str:
    ...
    """
    raw = ""
    # ``raw_text`` is Telethon's property over the wire ``message`` field;
    # fixtures may set either. ``text`` last: it is the markdown rendering.
    ...
    if raw:
        return raw
    # Rich Text Editor messages (#470) ship an empty wire text; the words are
    # in the block tree. Render it so the row is never silently empty.
...
def message_entities(message: object) -> list[dict] | None:
    """Serialized formatting entities for ``message_plain_text(message)``.

    The wire ``message.entities`` when the message carries text of its own;
    otherwise the spans derived from a Rich Text Editor block tree (#470), so
    the archived text and its entities always describe the same string.
    """
    serialized = serialize_message_entities(getattr(message, "entities", None))
    if serialized:
        return serialized
    rich = rich_message_of(message)
    if rich is None:
        return None
    return render_rich_message(rich)[1] or None


# ---------------------------------------------------------------------------
# Rich Text Editor messages (#470)
#
# Telegram's Premium composer sends a message whose wire ``message`` is empty
# and whose content lives in ``message.rich_message`` as Instant View page
# blocks. Flattening that tree into the archive's existing text + entities
# contract means search, export, the realtime feed and the viewer need no new
# path: headings become bold spans, quotes become blockquote spans, code
# becomes pre/code spans. The block tree itself is kept in raw_data so
# nothing is discarded. Everything is name-based so MagicMock fixtures stay
# inert, matching serialize_message_entities.
# ---------------------------------------------------------------------------
...
class _RichMessageRenderer:
    ...
    def render_text(self, node: object) -> None:
        ...
        if isinstance(node, str):
            ...
            for entity in serialize_message_entities(getattr(node, "entities", None)) or []:
                entity["offset"] += start
...
def render_rich_message(rich: object) -> tuple[str, list[dict]]:
    ...
    ``entities`` follow the serialize_message_entities shape (UTF-16 offsets)
