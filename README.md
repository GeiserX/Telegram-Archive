...
docker run -it --rm \
  ...
  drumsergio/telegram-archive:8.10.1 \
  python -m src auth
```
...
services:
  telegram-viewer:
    image: drumsergio/telegram-archive-viewer:8.10.1
    ports:
      - "127.0.0.1:8000:8000"
    environment:
...
* **A folder id, not a name.** Neither Telegram nor the viewer shows a folder's numeric id. After a scheduled backup has run with the folder visible to the account, the archive database lists them: `SELECT id, title FROM chat_folders;` (the `account_id` column tells accounts apart).
...
services:
  telegram-backup:
    image: drumsergio/telegram-archive:8.10.1
  telegram-viewer:
    image: drumsergio/telegram-archive-viewer:8.10.1
```
...
```bash
git pull
docker build -t drumsergio/telegram-archive:8.10.1 .
docker build -t drumsergio/telegram-archive-viewer:8.10.1 -f Dockerfile.viewer .
docker compose up -d
```
