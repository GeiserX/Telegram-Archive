...
services:
  telegram-backup:
    image: drumsergio/telegram-archive:8.13.0
  telegram-viewer:
    image: drumsergio/telegram-archive-viewer:8.13.0
```
...
```bash
git pull
docker build -t drumsergio/telegram-archive:8.13.0 .
docker build -t drumsergio/telegram-archive-viewer:8.13.0 -f Dockerfile.viewer .
docker compose up -d
```
