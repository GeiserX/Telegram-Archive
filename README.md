...
docker run -it --rm \
  ...
  drumsergio/telegram-archive:8.14.0 \
  python -m src auth
```
...
services:
  telegram-viewer:
    image: drumsergio/telegram-archive-viewer:8.14.0
    ports:
      - "127.0.0.1:8000:8000"
    environment:
...
services:
  telegram-backup:
    image: drumsergio/telegram-archive:8.14.0
  telegram-viewer:
    image: drumsergio/telegram-archive-viewer:8.14.0
```
...
```bash
git pull
docker build -t drumsergio/telegram-archive:8.14.0 .
docker build -t drumsergio/telegram-archive-viewer:8.14.0 -f Dockerfile.viewer .
docker compose up -d
```
