...
docker run -it --rm \
  ...
  drumsergio/telegram-archive:8.11.1 \
  python -m src auth
```
...
services:
  telegram-viewer:
    image: drumsergio/telegram-archive-viewer:8.11.1
    ports:
      - "127.0.0.1:8000:8000"
    environment:
...
services:
  telegram-backup:
    image: drumsergio/telegram-archive:8.11.1
  telegram-viewer:
    image: drumsergio/telegram-archive-viewer:8.11.1
```
...
```bash
git pull
docker build -t drumsergio/telegram-archive:8.11.1 .
docker build -t drumsergio/telegram-archive-viewer:8.11.1 -f Dockerfile.viewer .
docker compose up -d
```
